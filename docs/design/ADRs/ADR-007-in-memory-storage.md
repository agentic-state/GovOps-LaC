...
**Context**: the project plan §3 requires choosing the storage model for Phases 1–10. The choice is between **in-memory only** (consistent with v1.0; reseed-on-startup like the existing `DemoStore`) and a **persistence track** (SQLite/Postgres alongside the substrate work).
