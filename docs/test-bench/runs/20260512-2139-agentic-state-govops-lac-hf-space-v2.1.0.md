# Test bench run — 20260512-2139

| Field | Value |
|---|---|
| Target | `https://agentic-state-govops-lac.hf.space` |
| Target version | `2.1.0` |
| Started at (UTC) | `2026-05-12T21:39:52.691Z` |
| Duration | 1254.1s |
| Run status | `failed` |
| Journeys executed | 64 |
| Unattributed tests | 69 |

## Journey results (sorted by ID)

| ID | Title | Status | Tests | Duration | Browsers |
|---|---|---|---|---|---|
| J01 | renders /screen with a jurisdiction picker | PASS | 1 | 3.3s | chromium |
| J02 | /screen/ca renders the jurisdiction-specific form | PASS | 8 | 24.3s | chromium |
| J03 | renders the form with heading + jurisdiction selector | PASS | 4 | 13.6s | chromium |
| J04 | CA + job_loss renders the bounded-benefit timeline | PASS | 3 | 13.8s | chromium |
| J05 | no draft key in sessionStorage after filling the form | PASS | 4 | 14.4s | chromium |
| J06 | /cases lists at least the seeded demo case | PASS | 1 | 3.5s | chromium |
...
| J16 | renders an event timeline section | PASS | 1 | 3.7s | chromium |
| J17 | renders the headline comparison table with all six active jurisdictions | PASS | 4 | 17.5s | chromium |
| J18 | GET /api/programs/oas/compare returns rows for all 7 jurisdictions | PASS | 2 | 3.5s | chromium |
| J19 | GET /api/impact?citation=... returns an impact set | PASS | 3 | 3.3s | chromium |
| J20 | demo-seeded approvals queue is non-empty on first load | PASS | 1 | 3.4s | chromium |
| J21 | GET /api/config/versions returns a valid shape for any seeded key | PASS | 2 | 4.3s | chromium |
| J22 | /config/diff renders the diff route without an error boundary | PASS | 1 | 3.3s | chromium |
| J24 | draft lifecycle reflects in UI; resolve flips at the boundary | PASS | 1 | 5.5s | chromium |
| J25 | rejected draft moves out of the queue and is marked terminal | PASS | 1 | 2.7s | chromium |
| J26 | request-changes returns a pending draft to the author | PASS | 1 | 0.1s | chromium |
| J27 | /api/screen returns pre-supersession amount for 2025-06-01 evaluation | PASS | 5 | 2.5s | chromium |
| J28 | /config/prompts renders + lists prompt-domain ConfigValues | PASS | 1 | 3.3s | chromium |
| J29 | /config/prompts/{key}/{jur}/edit renders for an existing prompt | PASS | 1 | 3.4s | chromium |
| J30 | /encode renders + lists at least one batch fixture | PASS | 1 | 3.7s | chromium |
| J31 | /encode/new renders the new-batch form | PASS | 1 | 3.5s | chromium |
| J32 | encoder: approving a proposal locks the Approve/Modify/Reject buttons; Reopen replaces Annotate | PASS | 1 | 4.1s | chromium |
| J33 | emit-yaml endpoint accepts requests for known batch ids (or 404 cleanly) | PASS | 1 | 0.0s | chromium |
| J34 | GET /api/admin/federation/registry returns a registry shape | PASS | 2 | 7.4s | chromium |
| J35 | POST /federation/fetch/{publisher} for the first registered publisher (or skip) | SKIP | 1 | 0.0s | chromium |
| J36 | POST /federation/packs/{pub}/enable for the first registered publisher (or skip) | SKIP | 1 | 0.1s | chromium |
...
| J39 | /admin renders + surfaces operator runbook | PASS | 1 | 3.4s | chromium |
...
| J44 | page renders without an error boundary | PASS | 9 | 33.4s | chromium |
| J45 | walkthrough: 7-step paid-vacation scenario renders end to end | PASS | 1 | 3.3s | chromium |
| J46 | /policies renders the live registry with verdicts + provenance | PASS | 1 | 3.3s | chromium |
| J47 | approving via the UI removes the record from the queue without a hard reload | PASS | 3 | 11.3s | chromium |
| J48 | visiting /config/approvals/{id} for an already-approved record shows the resolved notice, not action buttons | PASS | 2 | 6.8s | chromium |
| M01 | a11y: / (WCAG 2.1 AA) | PASS | 16 | 136.7s | chromium |
| M02 | SSR head: /config renders a non-empty <title> | PASS | 9 | 0.5s | chromium |
| M03 | language selector lists all 6 GovOps locales | PASS | 3 | 14.8s | chromium |
| M04 | request-changes returns the draft to the author + record stays in queue with status=draft | FAIL | 24 | 73.5s | chromium |
| M05 | when the current user authored the draft, the action panel is disabled + shows the blocked alert | PASS | 2 | 6.9s | chromium |
| M06 | typing the shortcut while the panel is focused opens the approve dialog | PASS | 16 | 56.2s | chromium |
| M07 | filling the form and clicking Submit creates a draft, redirects to the timeline, and the new draft is visible on /config/approvals | PASS | 1 | 5.7s | chromium |
...
| M09 | typing into CodeMirror + clicking Save creates a draft on /config/approvals | PASS | 1 | 4.5s | chromium |
| M10 | typing a change then clicking Reset puts the original value back | PASS | 1 | 5.1s | chromium |
| M11 | clicking 'Show diff' reveals the Diff section; clicking 'Hide diff' hides it | PASS | 1 | 4.0s | chromium |
| M12 | deferred -- FixtureTestPanel requires an LLM provider key; not in this batch | SKIP | 1 | 0.0s | chromium |
| M13 | selecting 'pending' hides draft-only records and selecting 'all' restores them | PASS | 1 | 4.0s | chromium |
| M14 | typing the unique key in the search box hides every other row and leaves only the match | PASS | 1 | 3.3s | chromium |
...
| M16 | renders the timeline for a live seeded key with a Current Version section | PASS | 1 | 3.5s | chromium |
...
### M04 — request-changes returns the draft to the author + record stays in queue with status=draft

- **chromium** — smoke: /walkthrough renders
  ```
  Error: pageerror events on /walkthrough
