# Test bench run — 20260507-1528

| Field | Value |
|---|---|
| Target | `https://agentic-state-govops-lac.hf.space` |
| Target version | `2.1.0` |
| Started at (UTC) | `2026-05-07T15:28:53.485Z` |
| Duration | 440.9s |
| Run status | `failed` |
...
|---|---|---|---|---|---|
| J01 | renders /screen with a jurisdiction picker | PASS | 1 | 3.4s | chromium |
| J02 | /screen/ca renders the jurisdiction-specific form | PASS | 8 | 22.6s | chromium |
| J03 | renders the form with heading + jurisdiction selector | PASS | 4 | 13.2s | chromium |
| J04 | CA + job_loss renders the bounded-benefit timeline | PASS | 3 | 12.7s | chromium |
| J05 | no draft key in sessionStorage after filling the form | PASS | 4 | 13.9s | chromium |
| J06 | /cases lists at least the seeded demo case | PASS | 1 | 3.4s | chromium |
| J07 | evaluating demo-case-001 returns program_evaluations with citations | PASS | 2 | 3.8s | chromium |
...
| J16 | renders an event timeline section | PASS | 1 | 3.4s | chromium |
| J17 | renders the headline comparison table with all six active jurisdictions | PASS | 4 | 17.4s | chromium |
| J18 | GET /api/programs/oas/compare returns rows for all 7 jurisdictions | PASS | 2 | 3.3s | chromium |
| J19 | GET /api/impact?citation=... returns an impact set | PASS | 3 | 3.2s | chromium |
| J20 | demo-seeded approvals queue is non-empty on first load | PASS | 1 | 3.3s | chromium |
| J21 | GET /api/config/versions returns a valid shape for any seeded key | PASS | 2 | 3.5s | chromium |
| J22 | /config/diff renders the diff route without an error boundary | PASS | 1 | 3.1s | chromium |
| J24 | draft lifecycle reflects in UI; resolve flips at the boundary | PASS | 1 | 5.1s | chromium |
| J25 | rejected draft moves out of the queue and is marked terminal | PASS | 1 | 2.8s | chromium |
...
| J29 | /config/prompts/{key}/{jur}/edit renders for an existing prompt | PASS | 1 | 3.2s | chromium |
...
| J32 | encoder: approving a proposal locks the Approve/Modify/Reject buttons; Reopen replaces Annotate | PASS | 1 | 3.5s | chromium |
...
| J39 | /admin renders + surfaces operator runbook | PASS | 1 | 3.3s | chromium |
...
| J44 | page renders without an error boundary | PASS | 9 | 32.3s | chromium |
| J45 | walkthrough: 7-step paid-vacation scenario renders end to end | PASS | 1 | 3.2s | chromium |
| J46 | /policies renders the live registry with verdicts + provenance | PASS | 1 | 3.2s | chromium |
| J47 | /authority renders + GET /api/authority-chain returns a non-empty chain | PASS | 2 | 3.2s | chromium |
| J48 | home: modules + actors + walkthrough CTA all render | PASS | 1 | 3.1s | chromium |
| M01 | a11y: / (WCAG 2.1 AA) | PASS | 16 | 121.7s | chromium |
| M02 | SSR head: /config renders a non-empty <title> | FAIL | 10 | 0.6s | chromium |
| M03 | language selector lists all 6 GovOps locales | PASS | 2 | 6.9s | chromium |
| M04 | smoke: / renders | PASS | 22 | 60.4s | chromium |
| M05 | help drawer: Help button opens a sheet with route-aware content | PASS | 1 | 3.1s | chromium |
| M06 | breadcrumb: /walkthrough renders the layout-level breadcrumb | PASS | 15 | 47.2s | chromium |
